...
class AuthenticatorAssuranceLevel {
  ...
  static AuthenticatorAssuranceLevel? fromJson(dynamic value) => AuthenticatorAssuranceLevelTypeTransformer().decode(value);

  static List<AuthenticatorAssuranceLevel>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <AuthenticatorAssuranceLevel>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AuthenticatorAssuranceLevel.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
...
class AuthenticatorAssuranceLevelTypeTransformer {
  factory AuthenticatorAssuranceLevelTypeTransformer() => _instance ??= const AuthenticatorAssuranceLevelTypeTransformer._();
  ...
  AuthenticatorAssuranceLevel? decode(dynamic data, {bool allowNull = true}) {
    if (data != null) {
      switch (data) {
